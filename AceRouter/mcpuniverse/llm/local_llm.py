...
Works with any server exposing the OpenAI /v1/completions API,
...
class LocalLLMModel(BaseLLM):
    ...
    Attributes:
        ...
        alias: Aliases for the model — use "vllm_local" or "sglang_local".
    """
    ...
    def _generate(
        ...
        """
        Generates content using a local LLM server (vLLM/SGLang).
    ...
    def set_context(self, context: Context):
        ...
        self.config.api_key = context.env.get(
            "LOCAL_LLM_API_KEY", context.env.get("VLLM_API_KEY", self.config.api_key)
        )
        self.config.base_url = context.env.get(
            "LOCAL_LLM_BASE_URL", context.env.get("VLLM_BASE_URL", self.config.base_url)
        )
